"""
QdrantVectorStore is a vector store that uses Qdrant - https://qdrant.tech/
"""
...
from qdrant_client import QdrantClient, grpc, models  # type: ignore

from ..base import nlplatformBase
from ..utils import deterministic_uuid

SCROLL_SIZE = 1000


class QdrantVectorStore(nlplatformBase):
    """
    Vectorstore implementation using Qdrant - https://qdrant.tech/

    Args:
        - config (dict, optional): Dictionary of `Qdrant_VectorStore config` options.
            Defaults to None.
            ...
            - url: Either host or str of "Optional[scheme], host, Optional[port],
              Optional[prefix]". Eg. `"http://localhost:6333"`.
            - prefer_grpc: If `true` - use gPRC interface whenever possible in custom methods.
            - https: If `true` - use HTTPS(SSL) protocol. Default: `None`
            - api_key: api key for authentication in Qdrant Cloud. Default: `None`
            - timeout: Timeout for REST and gRPC api requests.
              Defaults to 5 seconds for REST and unlimited for gRPC.
            - path: Persistence path for QdrantLocal. Default: `None`.
            - prefix: Prefix to the REST URL paths.
              Example: `service/v1` will result in
              `http://localhost:6333/service/v1/{qdrant-endpoint}`.
            - n_results: Number of results to return from similarity search.
              Defaults to 10.
            - fastembed_model: Model to use for `fastembed.TextEmbedding`.
              Defaults to `"BAAI/bge-small-en-v1.5"`.
            - collection_params: Additional parameters to pass to
              `qdrant_client.QdrantClient#create_collection()` method.
            - distance_metric: Distance metric to use when creating collections.
              Defaults to `qdrant_client.models.Distance.COSINE`.
            - documentation_collection_name: Name of the collection to store documentation.
    ...
    def __init__(self, config: dict) -> None:
        nlplatformBase.__init__(self, config=config)
    ...
    def add_question_sql(self, question: str, sql: str) -> str:
        ...
        qid = deterministic_uuid(question_answer)
        self._client.upsert(
        ...
        return self._format_point_id(qid, self.sql_collection_name)

    def add_ddl(self, ddl: str, engine: str | None = None) -> str:
        ddlid = deterministic_uuid(ddl)
        self._client.upsert(
        ...
        return self._format_point_id(ddlid, self.ddl_collection_name)

    def add_documentation(self, documentation: str, **kwargs) -> str:
        did = deterministic_uuid(documentation)
        self._client.upsert(
        ...
        return self._format_point_id(did, self.documentation_collection_name)
    ...
    def remove_training_data(self, data_id: str) -> bool:
        try:
            pid, collection_name = self._parse_point_id(data_id)
            res = self._client.delete(collection_name, points_selector=[pid])
            if res:
                return True
            else:
                return False
    ...
    def embeddings_dimension(self):
        """
        This method is used to get the dimension of the embeddings.
        """
        return len(self.generate_embedding("TEST STRING"))
    ...
    def get_similar_question_sql(self, question: str) -> list:
        ...
        return [dict(point.payload) for point in results]
